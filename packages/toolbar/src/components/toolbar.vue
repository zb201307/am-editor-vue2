...
export default class Toolbar extends Vue {
    ...
    mounted(){
        ...
        if (isMobile) {
            ...
            document.addEventListener('scroll', this.hideMobileToolbar);
    ...
    unmounted(){
        ...
        if (isMobile) {
            ...
            document.removeEventListener('scroll', this.hideMobileToolbar);
    ...
    calcuMobileView(){
        ...
        this.mobileView.top = global.Math.max(document.body.scrollTop, document.documentElement.scrollTop) + (window.visualViewport.height || 0) - height
        }, 100);
    }
            ...
            group.forEach(item => {
                ...
                if (customItem) {
                    if (customItem.type === 'button') {
                        ...
                        else customItem.values = this.engine.command.queryState(
                                customItem.name,
                            );
                    }
                    if (customItem.type !== 'collapse')
                        customItem.disabled = customItem.onDisabled ? customItem.onDisabled() : !this.engine.command.queryEnabled(customItem.name);
                    else {
                        customItem.groups.forEach(group => group.items.forEach((item) => {
                        item.disabled = item.onDisabled ? item.onDisabled() : !this.engine.command.queryEnabled(item.name);
                        }))
